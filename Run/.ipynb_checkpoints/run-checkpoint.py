...
import pandas as pd
# How many horses?
# Race Distance? Furlongs, miles, or meters
# Type of Horses: (real, user generated, real/user generated, random)
# If not random...
# Add Your horses one at a time:
    # Type: (real, user generated)
    # If Real: just need name
    # else ask for attributes
    # Show some horse info: for user gen show ratings, for real show speed info
# Once horses are added: GENERATE ODDS using x simulations button
# Onclick, generates odds
# Maybe add section where user can input odds
# A visualization of the odds and expected probabilities are shown
class MyApp:
    def __init__(self):
    ...
    def build_page(self):
        ...
        if col1.button("Get Odds"):
            result = self.get_odds(col2)
            if result != None:
                col1.write(result)
        else:
            if st.session_state.ODDS is not None:
                col2.write('### Odds:')
